...
export class ProjectRepository {
  ...
  create(params: { name: string; path: string }): Project {
    if (!params.name || !params.path) {
      throw new EndeavorError(
        ErrorCode.PROJECT_INVALID_INPUT,
    ...
    if (existing) {
      throw new EndeavorError(
        ErrorCode.PROJECT_PATH_DUPLICATE,
    ...
    this.db
      ...
      .run(id, params.name, canonicalPath, now, now);

    return { id, name: params.name, path: canonicalPath, createdAt: now, updatedAt: now };
  }
  ...
  update(id: string, params: Partial<{ name: string }>): Project {
